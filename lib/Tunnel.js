...
const remoteRetryPolicy = generateExponentialBackoffRetryFn(1000, 4000, 10);
...
class Tunnel extends EventEmitter {
  ...
  async _init() {
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        const retryDelay = remoteRetryPolicy(attempt);
        debug(`tunnel server offline: ${err.message}, retrying in ${retryDelay}ms`);
  ...
  _establish(info) {
    ...
    this.tunnelCluster.once('open', this._handleTunnelOpen.bind(this));
    this.tunnelCluster.once('dead', this._handleTunnelRetry.bind(this));
  ...
  _handleTunnelRetry() {
    debug('tunnel is dead, starting retries');
    this.open();
  }
  ...
  close() {
    ...
    if (this.tunnelCluster) {
      ...
      this.tunnelCluster.removeAllListeners();
    }
    this.emit('close');
  }
}
