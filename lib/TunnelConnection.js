...
const generateExponentialBackoffRetryFn = (initialDelay, maxDelay, maxRetries = Infinity, jitter = true) => {
    return (attempt) => {
...
const remoteRetryPolicy = generateExponentialBackoffRetryFn(1000, 60000, 10);
...
class TunnelConnection extends EventEmitter {
    ...
    async open() {
        await this.connectRemoteWithRetry();
        await this.connectLocalWithRetry();
    }

    async connectRemoteWithRetry() {
        let attempt = 0;
        while (true) {
            try {
                ...
                if (err.message === 'Max retries reached') {
                    throw new Error('Failed to connect to remote after maximum retries');
                }
                const delay = remoteRetryPolicy(attempt);
                debug(`Remote connection failed. Retrying in ${delay}ms. Attempt: ${attempt}`);
    ...
    async connectLocal() {
        if (this.remote.destroyed) {
            debug('Remote connection destroyed. Aborting local connection.');
