...
const localRetryPolicyTimeout = generateExponentialBackoffRetryFn(1000, 10000);
  ...
  open() {
    ...
    const connLocal = (retries = 0) => {
      ...
      local.once('error', err => {
        ...
        const retryTimeout = localRetryPolicyTimeout(attempts);
        debug('retrying local connection in %ds', retryTimeout);
        setTimeout(() => connLocal(attempts), retryTimeout);
      });
